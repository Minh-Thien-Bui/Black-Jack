...
def blackJack():
    ...
    if player[0].getValue() == 0 or player[1].getValue() == 0:
        ...
        for i in range(len(player)):
            if player[i].getValue() == 1 and playerValue <= 11:
                player[i].setValue(11)

            playerValue = 0
            for count in range(len(player)):
                playerValue += player[count].getValue()

        for i in range(len(dealer)):
            if dealer[i].getValue() == 1 and dealerValue <= 11:
                dealer[i].setValue(11)

            dealerValue = 0
            for count in range(len(dealer)):
                dealerValue += dealer[count].getValue()
